...
$d="/var/www/html/Tabla/public/";   
$target_dir = $d."uploadsdoc/";
...
$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
  ...
  if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
   ...
   //insertar 
   if ($this->model->insertarchivo([ 'ide'=>$ide, 'archivo'=>$archivo])){
...
function upload0($param=null){
  $id = $param[0];
     // $solicitud=$this->model->get();   
        $this->view->solicitudid=$id;



      $this->view->render('solicitud/upload0');
       }

   function eliminardocadj($param = null){
   $id=$_POST['item'];
     if ($this->model->deletedocaj($id)){
      $this->view->mensaje='documentos Eliminado Correctamente';
   }else{
...
$this->verPaginacion('1');
 ...
 function verdoc($param=null){
        $id = $param[0];
       
      $sdoc=$this->model->getdoc($id);   
       //var_dump($sdoc);
        $this->view->sdoc=$sdoc;
     ...
     $this->view->render('solicitud/verdoc');
    }
...
$OTR="N";
   ...
   if ($this->model->update(['id'=>$id,'rut'=>$rut,'nombre'=>$nombre,'medico'=>$medico,'cirugia'=>$cirugia,'fechasp'=>$fechasp,'ODI'=>$ODI,'OD'=>$OD,'OI'=>$OI,'OTR'=>$OTR,'prevision'=>$prevision,'fechaag'=>$fechaag,'telefono_trabajo'=>$telefono_trabajo,'correo_electronico'=>$correo_electronico,'fecha_nacimiento'=>$fecha_nacimiento,'estado'=>$estado,'observaciones'=>$observaciones, 'fechaodi'=>$fechaodi, 'fechaod'=>$fechaod, 'fechaoi'=>$fechaoi, 'fechaotr'=>$fechaotr])) {
      ...
      $solicitud->fechaodi=$fechaodi;
      $solicitud->fechaod=$fechaod;
      $solicitud->fechaoi=$fechaoi;
      $solicitud->fechaotr=$fechaotr;
      $this->view->solicitud=$solicitud;
