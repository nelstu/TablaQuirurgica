...
$items = [];
 ...
 public function getuestado(){
  ...
  public function getpagestado($fe){
         ...
         public function getpag($iniciar,$autoporpag,$s,$order){
         $items = [];
         try{
     if ($s==null){
                 $query=$this->db->connect()->query("SELECT * FROM solicitud order by ".$order."  DESC LIMIT $iniciar,$autoporpag");
              }else{
                 $query=$this->db->connect()->query("SELECT * FROM solicitud  WHERE (id LIKE '%".$s."%')  OR (rut LIKE '%".$s."%')  OR (nombre LIKE '%".$s."%')  OR (medico LIKE '%".$s."%')  OR (cirugia LIKE '%".$s."%')  OR (ojo LIKE '%".$s."%')  OR (fechasp LIKE '%".$s."%')  OR (ODI LIKE '%".$s."%')  OR (OD LIKE '%".$s."%')  OR (OI LIKE '%".$s."%')  OR (OTR LIKE '%".$s."%')  OR (prevision LIKE '%".$s."%')  OR (fechaag LIKE '%".$s."%')  OR (telefono_trabajo LIKE '%".$s."%')  OR (correo_electronico LIKE '%".$s."%')  OR (fecha_nacimiento LIKE '%".$s."%')  OR (estado LIKE '%".$s."%')  OR (observaciones LIKE '%".$s."%')   order by ".$order." DESC LIMIT $iniciar,$autoporpag");
              }
             while($row=$query->fetch()){
                  $item = new Solicitud();
                $item->id     =$row['id'];
